...
import { renderPhotoViewer } from "./renderer.js";
...
let selectedRecords    = [];   // 타임라인에서 선택된 record 객체 배열
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (saved) {
    document.getElementById("workspaceStatus").innerHTML =
      `이전에 선택한 <b>'${saved.name}'</b> 폴더를 불러오시겠습니까?`;
    bannerActions.innerHTML = `
      ...
      <button id="newFolderBtn" class="btn-secondary"
              style="border-color:var(--btn-navy);color:var(--btn-navy);">새 폴더 연결하기</button>`;
    ...
    bannerActions.innerHTML =
      `<button id="selectFolderBtn" class="btn-primary">작업 폴더 선택 (필수)</button>`;
...
async function _finishFolderSetup() {
  document.getElementById("workspaceStatus").innerHTML =
    `연결된 작업 폴더: <b style="color:var(--btn-green);">${dirHandle.name}</b>`;
...
function _initModules() {
  initAddPatientModal({
    ...
    onSaved: () => { _updateTagDropdown(); _renderPatients(); },
    showAlert,
  });
  ...
  initEditPatientModal({
    ...
    onSaved: () => {
      _updateTagDropdown();
      _renderPatients();
      _openPatientDetail(activePatient);
    },
    showAlert,
  });
...
function _renderPatients() {
  ...
  const filtered = patientsData.filter(p => {
    ...
    const matchTags   = selectedTagsFilter.size === 0 ||
      Array.from(selectedTagsFilter).every(t => p.tags?.includes(t));
    return matchSearch && matchTags;
  });

  if (!filtered.length) {
    list.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><h3>조건에 맞는 환자가 없습니다.</h3></div>`;
    return;
  }
...
function _openPatientDetail(patient) {
  ...
  document.getElementById("detailChartNo").innerText =
    `진료번호: ${patient.chartNumber} | 초진일: ${patient.initialVisitDate || "미설정"}`;
  document.getElementById("detailTags").innerHTML =
    (patient.tags || []).map(t => `<span class="tag-badge">#${t}</span>`).join("");
...
document.getElementById("compareModeBtn").onclick = () => {
  ...
  if (isCompareMode) {
    btn.innerText = "비교 모드 (Before/After) ON";
    btn.classList.replace("btn-secondary", "btn-primary");
    btn.style.color = "white";
    document.getElementById("viewsContainer").className = "compare-layout";
    document.getElementById("secondaryViewPanel").style.display = "block";
    // 2번째 기록 자동 선택
  ...
  updateTimelineUI(selectedRecords, isCompareMode);
  _renderViewPanels();
};

function _resetCompareUI() {
  const btn = document.getElementById("compareModeBtn");
  btn.innerText = "비교 모드 (Before/After) OFF";
  btn.classList.replace("btn-primary", "btn-secondary");
  btn.style.color = "var(--btn-navy)";
  document.getElementById("viewsContainer").className = "compare-layout" === document.getElementById("viewsContainer").className
    ? "single-layout" : document.getElementById("viewsContainer").className;
  document.getElementById("viewsContainer").className   = "single-layout";
  document.getElementById("secondaryViewPanel").style.display = "none";
}
...
function _renderTimeline() {
  if (!activePatient.records?.length) {
    ...
    document.getElementById("photoViewerPrimary").innerHTML = "";
    document.getElementById("recordDatePrimary").innerHTML  = "날짜를 선택하세요";
    return;
  }
  ...
  if (!selectedRecords.length) {
    selectedRecords.push(activePatient.records[activePatient.records.length - 1]);
  }
  updateTimelineUI(selectedRecords, isCompareMode);
  _renderViewPanels();
}
...
// ──────────────────────────────────────────
// 뷰 패널 렌더링 (renderer.js 위임)
// ──────────────────────────────────────────
function _renderViewPanels() {
  if (selectedRecords[0]) _loadPanel(selectedRecords[0], "Primary");
  if (isCompareMode) {
    if (selectedRecords[1]) {
      _loadPanel(selectedRecords[1], "Secondary");
    } else {
      document.getElementById("recordDateSecondary").innerHTML    = "비교할 날짜 선택";
...
async function _loadPanel(record, prefix) {
  ...
  await renderPhotoViewer({
    ...
    onEdit:   (rec, idx) => openEditor({ record: rec, index: idx, dirHandle, patient: activePatient, showAlert }),
    onFullscreen: url => {
...
async function _deleteRecord(record) {
  ...
  if (!selectedRecords.length && activePatient.records.length) {
    selectedRecords.push(activePatient.records[activePatient.records.length - 1]);
  }
  await _savePatients();
  _renderTimeline();
  showAlert("진료 기록이 삭제되었습니다.");
}
...
document.getElementById("fullscreenViewer").onclick = e => {
